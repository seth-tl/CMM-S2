...
def Advect(Chi, V, t, dt, y0, spts, Integrator, eps = 1e-5, deriv = False):
    ...
    Chi_x_n1 = Hermite_proj_eps(Chi.Chi_x, eps1, eps2, eps3, eps4, yn)
    Chi_y_n1 = Hermite_proj_eps(Chi.Chi_y, eps1, eps2, eps3, eps4, yn)
    ...
    return Hermite_Map(H_phi_new, H_the_new)

def Initialize_Euler(u0, omega_0, dt, int0, XX, y0, spts, eps, xs, ys):

    U = velocity_interp(Vs = [u0], t0 = 0, dt = dt)
    inv_map1 = Advect(int0, U, 0, dt, y0, spts, Euler_Step, eps = eps)

    u1 = fspace.Biot_Savart(fft2(omega_0(inv_map1(XX))),xs, ys)
    U.Vs.append(u1)

    inv_map2 = Advect(int0, U, 0, dt, y0, spts, Improved_Euler, eps = eps)

    u2 = fspace.Biot_Savart(fft2(omega_0(inv_map2(XX))),xs, ys)
    ...
    u3 = fspace.Biot_Savart(fft2(omega_0(inv_map3(XX))),xs, ys)
...
def Initialize_horizontal_vector_field(u0, dt, int0, XX, y0, spts, eps, angle, rho, L):
    ...
    H_phi1, H_the1 = Advect(int0[0], int0[1], U, 0, dt, y0, spts, Euler_Step, eps = eps)

    qpts1 = [XX[0] + H_phi1.eval(XX[0], XX[1]), XX[1] + H_the1.eval(XX[0], XX[1])]

    u1 = gradient_field(angle, rho, s = dt, q_pts = qpts1, L = L)
    U.Vs.append(u1)

    H_phi2, H_the2 = Advect(int0[0], int0[1], U, 0, dt, y0, spts, Improved_Euler, eps = eps)

    qpts2 = [XX[0] + H_phi2.eval(XX[0], XX[1]), XX[1] + H_the2.eval(XX[0], XX[1])]

    u2 = gradient_field(angle, rho, s = dt, q_pts = qpts1, L = L)
    U.Vs[1] = u2

    #then one more
    H_phi3, H_the3 = Advect(H_phi2, H_the2, U, dt, dt, y0, spts, OneStepSO, eps = eps)

    qpts3 = [XX[0] + H_phi3.eval(XX[0], XX[1]), XX[1] + H_the3.eval(XX[0], XX[1])]

    u3 = gradient_field(angle, rho, s = 2*dt, q_pts = qpts1, L = L)
    U.Vs.append(u3)

    return U, [H_phi3, H_the3]
...
def gradient_field(theta, rho, s, q_pts, L):
    ...
    interpolant = defines initial density
    s = time parameter
    """

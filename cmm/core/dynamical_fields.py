...
def cosine_bell(phi,theta):
    ...
    #[phi, theta] = up2p(phi, theta, phi_c = 0., the_c = alpha)
    ...
    # arg3 = sin(the_c3-pi/2)*sin(theta-pi/2) + cos(the_c3-pi/2)*cos(theta-pi/2)*cos(phi-phi_c3)
    # arg4 = sin(the_c4-pi/2)*sin(theta-pi/2) + cos(the_c4-pi/2)*cos(theta-pi/2)*cos(phi-phi_c4)
    ...
    # r_p3 = arccos(arg3)
    # inds3 = np.where(r_p3 < R)
    # out3 = 0*theta
    # out3[inds3] = (h_0/2)*(1 + cos(r_p[inds3]*pi/R))
    #
    # r_p4 = arccos(arg4)
    # inds4 = np.where(r_p4 < R)
    # out4 = 0*theta
    # out4[inds4] = (h_0/2)*(1 + cos(r_p[inds4]*pi/R))
...
# ==============================================================================
# A collection of initial vorticity distributions:
def rotating_frame(phi, theta):
    # this is the 2*planetary vorticity with t = 1 being a full rotation
    return 4*pi*np.cos(theta)


def zonal_jet_creator(pert):
    pert = pert 

    def pZJ(phi, theta):
        #extra parameter gives the frequenct of perturbation
        beta2 = 12**2

...
# def project_stream_divfree(alms, L, grid, Method = "MWSS"):
#     """
#     function which outputs a spline interpolant defined over a structured
#     triangulation of the sphere allowing for O(1) querying.
...
#     # gradient operator
#     grad_vals = tan_proj(cross(u_num,[X,Y,Z]), [X,Y,Z])
#     vals = pysh.inverse(lm_array(alms, L), L, Spin = 0, Method = Method, Reality = True, backend = 'ducc', nthreads = 5)
...
#     # perform projection
#     #upsample by padding everything with zeros ?
#     interpolant = spline_interp_vec_divfree(grid = grid["mesh"], simplices = grid["simplices"],
#                  msimplices = grid["msimplices"], phi = grid["phis"], theta = grid["thetas"],
#                  vals = np.array(vals_n), grad_vals = [np.array(grad_vals_n[0]),
#                                            np.array(grad_vals_n[1]),
#                                            np.array(grad_vals_n[2])])

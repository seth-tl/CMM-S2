...
def Biot_Savart(omega_k, xs, ys):
    ...
    return [project_hermite(partial_y(psi_k), len(xs)), project_hermite(partial_x(-psi_k), len(ys))]

def Biot_Savart_scalar(omega_k, xs, ys):
    psi_k = Poisson_Solve(omega_k)
    return project_hermite(psi_k, len(xs))
